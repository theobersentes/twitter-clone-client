...
import type { GetUserByIdQuery, User } from "@/gql/graphql"
import { FollowUser, UnFollowUser } from "@/actions/follow_unfollow"
import { type QueryObserverResult, type RefetchOptions, useQueryClient } from "@tanstack/react-query"
...
export default function UserInfo({ user, currentUser, refetch }: Props) {
  ...
  const handleFollowUser = useCallback(
    async () => await FollowUser(currentUser.id, user.id, setButtonLoading, queryClient),
    [user],
  )

  const handleUnfollowUser = useCallback(
    async () => await UnFollowUser(currentUser.id, user.id, setButtonLoading, queryClient),
    [user],
  )
  ...
  return (
    <div className="p-4 border border-zinc-800 space-y-4 bg-zinc-950">
      ...
      {user.bio && <p className="text-zinc-300  transition-all cursor-pointer">{user.bio}</p>}
